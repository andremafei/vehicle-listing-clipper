...
`;function pr(e){const t=document.createElement("div");t.id="lead-crm-filler-root";const n=t.attachShadow({mode:"open"}),r=document.createElement("style");r.textContent=fr;const o=document.createElement("div");o.className="lcf-panel";const i=document.createElement("div");i.className="lcf-header";const a=document.createElement("div");a.className="lcf-title",a.textContent="CRM · Leads";const s=document.createElement("span");s.className="lcf-badge",s.textContent="CRM";const f=document.createElement("button");f.type="button",f.className="lcf-mini",f.textContent="–",i.append(a,s,f);const d=document.createElement("div");d.className="lcf-body";const l=document.createElement("div");l.className="lcf-hint",l.textContent="Cole o texto do Clipper (com LEAD_CLIP_V1) ou leia a área de transferência.";const c=document.createElement("textarea");c.className="lcf-textarea",c.placeholder="Cole aqui o texto do Clipper…";const u=document.createElement("div");u.className="lcf-summary",u.hidden=!0;const b=document.createElement("div");b.className="lcf-section-label",b.textContent="Leads encontrados",b.hidden=!0;const p=document.createElement("ul");p.className="lcf-matches";const y=document.createElement("div");y.className="lcf-row";const m=document.createElement("button");m.type="button",m.className="lcf-btn",m.textContent="Ler área de transferência";const h=document.createElement("button");h.type="button",h.className="lcf-btn",h.textContent="Analisar texto";const v=document.createElement("button");v.type="button",v.className="lcf-btn lcf-btn-primary",v.textContent="Verificar cadastro",v.disabled=!0,y.append(m,h,v);const x=document.createElement("div");x.className="lcf-row";const w=document.createElement("button");w.type="button",w.className="lcf-btn lcf-btn-danger",w.textContent="Criar lead",w.disabled=!0,w.hidden=!0,x.append(w);const k=document.createElement("div");k.className="lcf-status",k.dataset.tone="",k.textContent="Aguardando dados do anúncio.",d.append(l,c,u,b,p,y,x,k),o.append(i,d),n.append(r,o),document.documentElement.append(t);let S=!1;f.addEventListener("click",()=>{S=!S,d.hidden=S,f.textContent=S?"+":"–"});let _=!1,R=0,O=0;return i.addEventListener("pointerdown",L=>{if(L.target===f)return;_=!0;const $=o.getBoundingClientRect();R=L.clientX-$.left,O=L.clientY-$.top,i.setPointerCapture(L.pointerId)}),i.addEventListener("pointermove",L=>{_&&(o.style.left=`${L.clientX-R}px`,o.style.top=`${L.clientY-O}px`,o.style.right="auto",o.style.bottom="auto")}),i.addEventListener("pointerup",()=>{_=!1}),m.addEventListener("click",()=>e.onReadClipboard()),h.addEventListener("click",()=>e.onParseText(c.value)),c.addEventListener("paste",()=>{setTimeout(()=>e.onParseText(c.value),0)}),v.addEventListener("click",()=>e.onVerify()),w.addEventListener("click",()=>e.onCreate()),{setBadge(L){s.textContent=L},setStatus(L,$=""){k.textContent=L,k.dataset.tone=$||""},setText(L){c.value=L},getText(){return c.value},setSummary(L){if(!L){u.hidden=!0,u.textContent="";return}u.hidden=!1,u.innerHTML=L},setVerifyEnabled(L){v.disabled=!L},setCreateVisible(L,$=!0){w.hidden=!L,w.disabled=!$},setMatches(L,$){p.replaceChildren(),b.hidden=L.length===0;for(const N of L){const B=document.createElement("li"),T=document.createElement("div");T.className="lcf-match";const V=document.createElement("div");V.className="lcf-match-title",V.textContent=N.title;const J=document.createElement("div");J.className="lcf-match-sub",J.textContent=N.subtitle;const M=document.createElement("button");M.type="button",M.className="lcf-match-open",M.textContent="Abrir lead →",M.addEventListener("click",()=>$(N.id)),T.append(V,J,M),B.append(T),p.append(B)}},clearMatches(){p.replaceChildren(),b.hidden=!0},destroy(){t.remove()}}}function mr(){let e=null,t=null,n=!1,r=null;function o(){const m=bn();return t?.setBadge(m.label),m}function i(m){const h=Oo(m);if(t?.clearMatches(),t?.setCreateVisible(!1),!h.ok){e=null,t?.setSummary(null),t?.setVerifyEnabled(!1),t?.setStatus(`Falha ao analisar o texto: ${h.error}`,"error");return}e=h.payload,t?.setText(m),t?.setVerifyEnabled(!0),t?.setSummary([`<div><strong>ID</strong> ${ue(e.id)}</div>`,`<div><strong>Placa</strong> ${ue(e.plate||"—")}</div>`,`<div><strong>Telefone</strong> ${ue(e.phone||"—")}</div>`,`<div><strong>Veículo</strong> ${ue([e.make,e.model,e.year].filter(Boolean).join(" · ")||"—")}</div>`,`<div><strong>URL</strong> ${ue(e.url||"—")}</div>`].join("")),o(),t?.setStatus("LEAD_CLIP_V1 encontrado. Clique em Verificar cadastro.","ok")}async function a(){try{const m=await navigator.clipboard.readText();t?.setText(m),i(m)}catch(m){const h=m instanceof Error?m.message:"área de transferência indisponível";t?.setStatus(`Não foi possível ler a área de transferência (${h}). Cole o texto e use Analisar texto.`,"warn")}}function s(m){i(m)}async function f(){if(!e||n)return;if(o().backend==="leaddesk"){await d();return}await l()}async function d(){n=!0,t?.setStatus("Verificando no LeadDesk…"),t?.clearMatches(),t?.setCreateVisible(!1);try{const m=ne(e.plate),h=te(e.phone);let v=[];if(m&&(v=await ir(m)),v.length===0&&h&&(v=await ar(h)),!m&&!h){t?.setStatus("Os dados não têm placa nem telefone.","warn");return}if(v.length===0){t?.setStatus("Nenhum cadastro no LeadDesk. É possível criar um novo lead.","warn"),t?.setCreateVisible(!0,!0);return}const x=v.map(w=>({id:w.id,title:`Lead ${w.plate||w.id}`,subtitle:`${w.phone||"—"} · ${[w.make,w.model,w.year].filter(Boolean).join(" · ")||"—"} · ${w.leadStatus||""} · ${w.updatedAt||""}`.trim()}));t?.setMatches(x,w=>{location.assign(`/crm/leads/${w}`)}),t?.setStatus(x.length===1?"1 lead encontrado. Use Abrir lead ou crie outro.":`${x.length} leads encontrados. Use Abrir lead ou crie outro.`,"ok"),t?.setCreateVisible(!0,!0)}catch(m){const h=m instanceof Error?m.message:"erro";t?.setStatus(`Erro na verificação LeadDesk: ${h}`,"error")}finally{n=!1}}async function l(){n=!0,t?.setStatus("Verificando no CRM…"),t?.clearMatches(),t?.setCreateVisible(!1);try{const m=ne(e.plate),h=te(e.phone);let v;if(m)v=await de({plate:m}),v.ok&&Array.isArray(v.data)&&v.data.length===0&&h&&(v=await de({phone:h}));else if(h)v=await de({phone:h});else{t?.setStatus("Os dados não têm placa nem telefone.","warn");return}if(!v.ok){t?.setStatus(`Falha na verificação (HTTP ${v.status}). Está autenticado no CRM?`,"error");return}const x=Array.isArray(v.data)?v.data:[];if(x.length===0){t?.setStatus("Nenhum cadastro para esta placa/telefone. É possível criar o lead.","warn"),t?.setCreateVisible(!0,!0);return}const w=[];for(const S of x){const _=S?.purchaseLead?.id;if(_)w.push({id:_,title:`Lead #${_}`,subtitle:`${S.name||""} ${S.firstSurname||""} · ${S.contact?.primaryPhone||""} · ${S.purchaseLead?.statusName||""}`.trim()});else if(S?.id){const O=(await Qo(S.id)).data?.results||[];for(const L of O)w.push({id:L.id,title:`Lead #${L.id}`,subtitle:`Placa ${L.plate||"—"} · ${L.status?.name||""} · ${L.lastAction||""}`.trim()});O.length===0&&w.push({id:`client:${S.id}`,title:`Cliente #${S.id} (sem lead de compra)`,subtitle:`${S.name||""} ${S.firstSurname||""} · ${S.contact?.primaryPhone||""}`.trim()})}}const k=w.filter(S=>String(S.id).match(/^\d+$/));t?.setMatches(k.length?k:w,S=>{if(String(S).startsWith("client:")){t?.setStatus("Cliente sem lead de compra. É possível criar um novo lead.","warn"),t?.setCreateVisible(!0,!0);return}location.assign(`/main/lead-tasacion/${S}`)}),t?.setStatus(k.length===1?"1 lead encontrado. Use Abrir lead ou crie outro.":k.length>1?`${k.length} leads encontrados. Use Abrir lead ou crie outro.`:"Cliente encontrado sem lead válido para abrir. É possível criar um lead.",k.length?"ok":"warn"),t?.setCreateVisible(!0,!0)}catch(m){const h=m instanceof Error?m.message:"erro";t?.setStatus(`Erro na verificação: ${h}`,"error")}finally{n=!1}}async function c(){if(!e||n)return;if(o().backend==="leaddesk"){await u();return}await b()}async function u(){if(!te(e.phone)&&!ne(e.plate)){t?.setStatus("É necessário telefone ou placa para criar.","warn");return}if(confirm("Criar lead no LeadDesk local com os dados copiados?")){n=!0,t?.setStatus("Criando no LeadDesk…");try{const h=await sr(e);t?.setStatus(`Lead ${h} criado. Abrindo a página…`,"ok"),location.assign(`/crm/leads/${h}`)}catch(h){const v=h instanceof Error?h.message:"erro";t?.setStatus(`Erro ao criar no LeadDesk: ${v}`,"error")}finally{n=!1}}}async function b(){const m=te(e.phone);if(!m&&!ne(e.plate)){t?.setStatus("É necessário telefone ou placa para criar.","warn");return}if(confirm("Criar cliente/lead no CRM com os dados copiados?")){n=!0,t?.setStatus("Criando no CRM…");try{const h=await Jo();if(!h.ok||!h.data?.id){t?.setStatus(`Falha de autenticação (HTTP ${h.status}). Faça login no CRM.`,"error");return}const v=h.data,x=await Zo(),w=Array.isArray(x.data)&&x.data[0]?.value||z.id_local_actual,[k,S,_,R]=await Promise.all([Te("estado_lead_compra"),Te("origen_lead_compra"),Te("contacto"),Te("marcas_comerciales",v.id)]),O={estado:Re(k.data,"Avaliação")||z.estado,origen:Re(S.data,"Captación")||z.origen,contacto:Re(_.data,"Whatsapp")||z.forma_contacto,marca:Re(R.data,"")||z.marca_comercial};let L=null;if(m){const T=await de({phone:m});T.ok&&Array.isArray(T.data)&&T.data[0]?.id&&(L=T.data[0].id)}if(!L){const T=await er(lr(e));if(T.status===409)L=(await de({phone:m||void 0,plate:ne(e.plate)||void 0})).data?.[0]?.id;else if(T.ok&&T.data?.resourceId)L=T.data.resourceId;else{t?.setStatus(`Falha ao criar cliente (HTTP ${T.status}): ${JSON.stringify(T.data)}`,"error");return}}if(!L){t?.setStatus("Não foi possível obter clientId.","error");return}const $=cr({clip:e,clientId:L,me:v,localId:w,filters:O}),N=await tr($);if(!N.ok){t?.setStatus(`Falha create_lead_compra (HTTP ${N.status}): ${JSON.stringify(N.data)}`,"error");return}const B=N.data?.id_lead;if(!B){t?.setStatus(`Resposta inesperada: ${JSON.stringify(N.data)}`,"error");return}t?.setStatus(`Lead ${B} criado. Abrindo a página…`,"ok"),location.assign(`/main/lead-tasacion/${B}`)}catch(h){const v=h instanceof Error?h.message:"erro";t?.setStatus(`Erro ao criar: ${v}`,"error")}finally{n=!1}}}function p(){if(t)return t;t=pr({onReadClipboard:a,onParseText:s,onVerify:f,onCreate:c}),o(),window.addEventListener("popstate",o),r=new MutationObserver(()=>o());const m=document.getElementById("app")||document.body;return m&&r.observe(m,{childList:!0,subtree:!0}),a(),t}function y(){window.removeEventListener("popstate",o),r?.disconnect(),r=null,t?.destroy(),t=null,e=null}return{mount:p,destroy:y,refreshContext:o}}function ue(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const Qe="__LEAD_CRM_FILLER_INITIALIZED__",gr="lead-crm-filler-root";function hr(){return typeof window>"u"||typeof document>"u"?{started:!1,reason:"no-dom"}:bn().backend!=="none"?br():yr()}function br(){if(window[Qe])return{started:!1,reason:"already-initialized"};if(document.getElementById(gr))return window[Qe]=!0,{started:!1,reason:"panel-exists"};window[Qe]=!0;const e=mr(),t=()=>{e.mount()};return document.body?t():document.addEventListener("DOMContentLoaded",t,{once:!0}),{started:!0,reason:"crm"}}function yr(){if(window[De])return{started:!1,reason:"already-initialized"};if(document.getElementById(he))return window[De]=!0,{started:!1,reason:"panel-exists"};window[De]=!0;const e=Yo(),t=()=>{e.mount(document.body)};return document.body?t():document.addEventListener("DOMContentLoaded",t,{once:!0}),{started:!0}}hr()})();
