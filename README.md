...
2. **Plate** — discovers gallery image URLs, then downloads and scans **one image at a time** at **medium resolution** first (Apollo CDN `;s=1440x0` when applicable, decode long-edge cap 1440). Stops early when a Portuguese plate reaches **≥90%** mean OCR confidence. If the whole gallery stays below 90%, **only** images that already showed a low-confidence plate are re-scanned at **high resolution** (full CDN URL, no decode cap). Models are cached in IndexedDB after the first download. Runs even when the tab is in the background. When a plate is found, the panel records the **1-based gallery image index**, URL, and confidence (also stored in listing-cache metadata). See [DECISIONS.md](DECISIONS.md) (“Interleaved download + scan”) for the speed rationale.
